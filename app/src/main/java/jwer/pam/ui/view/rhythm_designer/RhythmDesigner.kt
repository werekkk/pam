...
import jwer.pam.utils.getChildMaxEnd
import jwer.pam.utils.setMargins
...
class RhythmDesigner(context: Context, attributeSet: AttributeSet) : LinearLayout(context, attributeSet) {
    ...
    private fun createViews() {
        ...
        rhythmLines.forEach { line -> blockLines.add(
            BlockLine(
                context,
                meter,
                line,
        ...
        blockLines.forEach { line -> run{
            line.layoutParams = RelativeLayout.LayoutParams(line.getChildMaxEnd(),
                BLOCK_HEIGHT
            )
            line.setMargins(
                MARGIN,
